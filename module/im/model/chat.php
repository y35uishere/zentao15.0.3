...
class chat extends model
{
    ...
    {
        $limit = isset($this->config->keepDismissedGroupMonth) ? $this->config->keepDismissedGroupMonth : 3;
        $chats = $this->dao->select('chat.*, cu.star, cu.hide, cu.mute, cu.freeze, cu.category')
    ...
    public function isAdmin($chat, $userID)
    {
        ...
        $creatorID = $this->dao->select('id')->from(TABLE_USER)->where('account')->eq($chat->createdBy)->fetch('id');
