...
class im extends control
{
    ...
    public function userLogin($account = '', $password = '', $options = array(), $userID = 0, $version = '', $device = 'desktop')
    {
        ...
        $userChatList = $this->im->chat->getListByUserID($user->id);
        $chatList     = $this->im->getChatListOutput($user->id, true, $userChatList);
        $conferences  = $this->im->getOpenConferencesOutput($user->id, true, $userChatList);
     ...
     * @return array
     */
    public function userGetListByDept($deptID, $pager, $orderBy, $exclude = array(), $userID = 0)
    {
        ...
        $list = $this->im->user->getIDListByDept($deptID, $exclude, $pager, $orderBy);
        $idList = array();
    ...
    public function chatAddAdmins($gid = '', $admins = array(), $userID = 0)
    {
        ...
        if($chat->createdBy != $account) return $this->im->sendOutput(array('result' => 'fail', 'message' => $this->lang->im->notGroupCreator), 'messageResponsePack');
